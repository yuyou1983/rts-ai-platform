...
`environment.json` is the contract between harness-creator and harness-executor. It describes everything the executor needs to know to start the application, set up dependencies, and run verification — but it does NOT define what to verify. Verification configuration (`verify.json`) is dynamically generated by harness-executor at task runtime.

> **Key principle**: harness-creator answers "what does this project need to run?" — harness-executor answers "what should we check after making changes?"
...
# Generated by harness-creator, consumed by harness-executor
set -euo pipefail
