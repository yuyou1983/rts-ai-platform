...
@export var max_zoom: float = 96.0
...
var keyboard_screen_per_second: float = 0.72
var edge_screen_per_second: float = 0.62
var edge_ramp_px: float = 20.0
var gameplay_visible_world_width: float = 32.0
var inspection_visible_world_width: float = 22.0
var overview_visible_world_width: float = 48.0
...
func _ready() -> void:
	...
	if _config.has("camera"):
		...
		edge_ramp_px = float(c.get("edge_ramp_px", edge_ramp_px))
		gameplay_visible_world_width = float(c.get("gameplay_visible_world_width", gameplay_visible_world_width))
		inspection_visible_world_width = float(c.get("inspection_visible_world_width", inspection_visible_world_width))
		overview_visible_world_width = float(c.get("overview_visible_world_width", overview_visible_world_width))
...
static func calculate_zoom_for_visible_width(viewport_width: float, visible_world_width: float) -> float:
	return viewport_width / maxf(visible_world_width, 1.0)


static func normalize_movement_direction(direction: Vector2) -> Vector2:
...
func setup(camera: Camera2D) -> void:
	...
	var start_zoom: float = calculate_zoom_for_visible_width(viewport_width, gameplay_visible_world_width)
	start_zoom = clampf(start_zoom, min_zoom, max_zoom)
...
func _handle_keyboard_movement(delta: float) -> void:
	...
	if Input.is_action_pressed("move_camera_right"):
		direction.x += 1.0
	_camera.position += normalize_movement_direction(direction) * world_speed * delta

# ── Edge Scroll ──────────────────────────────────────────────────────────────
func _handle_edge_scroll(delta: float) -> void:
	if _middle_dragging or not DisplayServer.window_is_focused():
		return
...
func _handle_zoom_lerp(delta: float) -> void:
	if _target_zoom.x < min_zoom:
		_target_zoom = Vector2(min_zoom, min_zoom)
...
func _clamp_camera() -> void:
	...
	# A map smaller than the viewport remains centered instead of creating an
	# inverted clamp range.
	_camera.position.x = map_px_w * 0.5 if half_vp_x * 2.0 >= map_px_w else clampf(_camera.position.x, half_vp_x, map_px_w - half_vp_x)
	_camera.position.y = map_px_h * 0.5 if half_vp_y * 2.0 >= map_px_h else clampf(_camera.position.y, half_vp_y, map_px_h - half_vp_y)
